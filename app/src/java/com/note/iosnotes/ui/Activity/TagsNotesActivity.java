...
public class TagsNotesActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private int mPinNoteSelectedPos;
    private int tagId;
    private int itemId;
    ...
    private void setAdapters() {
        pinnotesAdapter = new AllNotesAdapter(context, PinnedNotesList, StrTagName, new AllNotesAdapter.setNotesList() {
            public void onNoteSelected(int i) {
                mPassword = new Pref(getApplicationContext()).getString(Constant.STR_PASSWORD);
                if (!isEditMode) {
                    mPinNoteSelectedPos = i;
                    isClick = "isPin";
                    System.out.println("------ IDdds : " + mPinNoteSelectedPos);
                    ...
                    if (itemId == 1) {
                        ...
                        System.out.println("------ IDdd : " + (PinnedNotesList.get(mPinNoteSelectedPos)).toString());
                        openNote(true);
                    } else {
                        isType = "IsPin";
                        confirmPassword();
    ...
    public void getListNoteInFolder() {
        ...
        if (i == 0) {
            NotesList = helper.getIsDelete(0, 0);
            PinnedNotesList = helper.getIsDelete(0, 1);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                Collections.sort(NotesList, Comparator.comparing(Note::getDateTimeMills).reversed()
                        .thenComparing(Note::getIntPinOrder).reversed());
            }
            ...
            PinnedNotesList = helper.getFolderIdWithIsDeleteOrNot(tagId, 0, 1);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                Collections.sort(NotesList, Comparator.comparing(Note::getDateTimeMills).reversed()
                        .thenComparing(Note::getIntPinOrder).reversed());
            }
        } else {
            NotesList = helper.getIsDelete(1, 0);
            PinnedNotesList = helper.getIsDelete(1, 1);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                Collections.sort(NotesList, Comparator.comparing(Note::getDateTimeMills).reversed());
            }
        }
    ...
    private void showDialogCreateNewTag(final ArrayList<Note> realmList) {
        AddNewTagDialog addNewTagDialog = new AddNewTagDialog(context, getResources().getString(R.string.new_folder), getResources().getString(R.string.move_to_this_folder), getResources().getString(R.string.create_button), new AddNewTagDialog.CreateNewTagListeners() {
    ...
    private void GotoSort() {
        if (PinnedNotesList.size() >= 0 || NotesList.size() >= 0) {
            new IOSSheetDialog.Builder(context)
                    .setCancelText((CharSequence) getResources().getString(R.string.cancel))
                    .setData(new IOSSheetDialog.SheetItem[]
                            {new IOSSheetDialog.SheetItem(getResources().getString(R.string.sort_alphabetic_ascending)
                                    , IOSSheetDialog.SheetItem.BLUE),
                                    new IOSSheetDialog.SheetItem(getResources().getString(R.string.sort_alphabetic_descending)
                                            , IOSSheetDialog.SheetItem.BLUE),
                                    new IOSSheetDialog.SheetItem(getResources().getString(R.string.sort_alphabetic_time)
    ...
    public void showDialogRecoveryNote() {
        new IOSSheetDialog.Builder(context).setTitle(getResources().getString(R.string.choose_action)).setCancelText((CharSequence) getResources().getString(R.string.action_cancel)).setData(new IOSSheetDialog.SheetItem[]{new IOSSheetDialog.SheetItem(getResources().getString(R.string.action_recover), IOSSheetDialog.SheetItem.BLUE), new IOSSheetDialog.SheetItem(getResources().getString(R.string.action_delete_permanent), IOSSheetDialog.SheetItem.RED)}, (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialogInterface, int i) {
                dialogInterface.dismiss();
                System.out.println("----- mpin : " + mPinNoteSelectedPos + " --note : " + mNoteSelectedPos + " - isClick : " + isClick);
                System.out.println("----- mpin : " + PinnedNotesList.size() + " --note : " + NotesList.size() + " - isClick : " + isClick);
                if (i == 0) {
                    if (NotesList != null && NotesList.size() > 0 && isClick.equalsIgnoreCase("isNote")) {
                        recoveryNote(NotesList.get(mNoteSelectedPos));
                    }
                    if (PinnedNotesList != null && PinnedNotesList.size() > 0 && isClick.equalsIgnoreCase("isPin")) {
                        recoveryNote(PinnedNotesList.get(mPinNoteSelectedPos));
                    }
                    ...
                    if (PinnedNotesList != null && PinnedNotesList.size() > 0 && isClick.equalsIgnoreCase("isPin")) {
                        deleteNotePermanently(PinnedNotesList.get(mPinNoteSelectedPos));
                    }
                    getListNoteInFolder();
                    notesAdapter.updateData(NotesList);
                    pinnotesAdapter.updateData(PinnedNotesList);
...
//            note.setPinnedOrNot(note.isPinnedOrNot());
//            note.setIntPinOrder(0);
    ...
    private void openNote(boolean b) {
        if (!b) {
            ...
            int openId = (PinnedNotesList.get(mPinNoteSelectedPos)).getTagId();
            Intent intent = new Intent(context, NewCreateNotesActivity.class);
            intent.putExtra(Constant.TAGS_NAME, StrTagName);
            intent.putExtra(Constant.TAG_FOLDER_ID, openId);
            intent.putExtra(Constant.NOTE_ACTION_CODE, 1);

            intent.putExtra(Constant.TAGS_ID, ((Note) PinnedNotesList.get(mPinNoteSelectedPos)).getId());
            startActivityForResult(intent, VIEW_NOTE_CODE);
