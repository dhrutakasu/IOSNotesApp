...
public class NewCreateNotesActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initViews() {
        ...
        IvShareNote = (ImageView) findViewById(R.id.IvShareNote);
    ...
    private void getIntents() {
        ...
        if (widgetId != -1 && (noteHelper.getWidgetId(widgetId)) != null) {
            note = noteHelper.getWidgetId(widgetId);
            int TagFolderId = note.getTagId();
    ...
    private void initListeners() {
        ...
        IvShareNote.setOnClickListener(this);
...
//                        }
    ...
    private void NoteSave() {
        ...
        if (actionCode == 0) {
            ...
            count = notesCount;
            System.out.println("----- countsss count: " + count);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (manager != null && manager.isRequestPinAppWidgetSupported()) {
                new Handler().postDelayed(() -> {
                    ...
                    if (noteActionCode == 0) {
                        int notesCount = (int) noteHelper.getNotesCount();
                        System.out.println("----- count : " + notesCount);
                        if (notesCount != 0) {
    ...
    private void GotoMore() {
        ...
        BottomView bottomView = new BottomView(this, getSupportFragmentManager(), str, isPinnedOrNot, isLockedOrNot, countWords(EdtCreateNote.getText().toString()), titleBold, titleItalic, titleUnderline, titleStrike, contentBold, contentItalic, contentUnderline, contentStrike, align, new BottomView.setBottomMenu() {
            ...
            public void onPinNote() {
                ...
                if (noteActionCode == 0) {
                    String title = EdtCreateNoteTitle.getText().toString();
                    String content = EdtCreateNote.getText().toString();
                    notesCount = (int) noteHelper.getNotesCount();
                    if (notesCount >= 0) {
                        System.out.println("----- count : " + notesCount);
                        if (!noteHelper.checkRecordExist(title, content)) {
                            NoteSave();
...
//                        }
        ...
        WindowManager.LayoutParams lp = bottomView.getWindow().getAttributes();
        Window window = bottomView.getWindow();
        lp.copyFrom(window.getAttributes());
        lp.width = WindowManager.LayoutParams.MATCH_PARENT;
        lp.height = WindowManager.LayoutParams.WRAP_CONTENT;
        window.setAttributes(lp);
        lp.gravity = Gravity.TOP;
        bottomView.getWindow().setBackgroundDrawableResource(android.R.color.transparent);
    }
    ...
    private void GotoConfirmDeleteNote() {
        ...
        String title = EdtCreateNoteTitle.getText().toString();
        if (noteActionCode == 1) {
            str3 = String.format(getResources().getString(R.string.note_keep_30_day), title);
            str2 = getResources().getString(R.string.delete_note);
            str = getResources().getString(R.string.delete_button);
        } else {
            str3 = String.format(getResources().getString(R.string.confirm_discard_note), title);
...
//                PinOrder = 0;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        System.out.println("-- uRI  :qq " + requestCode + " resultCode: " + resultCode + " intent: " + intent);
        switch (requestCode) {
